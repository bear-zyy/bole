...
- (void)viewDidLayoutSubviews
{
    ...
    _navView.frame = CGRectMake(0, 0, PYSEARCH_REALY_SCREEN_WIDTH, 64);
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
    ...
    [backBut addTarget:self action:@selector(cancelDidClick) forControlEvents:UIControlEventTouchUpInside];
    backBut.frame = CGRectMake(PYSEARCH_REALY_SCREEN_WIDTH- 70, 30, 30, 30);
    [_navView addSubview:backBut];
...
//    UIView *titleView = [[UIView alloc] init];
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:_navView.frame];
    [_navView addSubview:searchBar];
//    self.navigationItem.titleView = titleView;
    searchBar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    searchBar.placeholder = @"关键字搜索圈子话题";
    searchBar.backgroundImage = [UIImage imageNamed:@"clearImage"];//[UIImage imageNamed:@""];

...
//    [self.loadingView startAnimating];//@zyy
    
    self.startPalyOrPauseBut = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.startPalyOrPauseBut setImage:[UIImage imageNamed:@"sike_icon_stop"] forState:UIControlStateNormal];
    [self.contentView addSubview:self.startPalyOrPauseBut];
    [self.startPalyOrPauseBut mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.contentView);
    }];
    
    [self.startPalyOrPauseBut addTarget:self action:@selector(playingClick) forControlEvents:UIControlEventTouchUpInside];
...
-(void)playingClick{
    ...
    self.startPalyOrPauseBut.hidden = YES;
